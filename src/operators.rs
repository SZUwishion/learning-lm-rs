...
pub fn add<T: Copy + Clone + Default>(y: &mut Tensor<T>, x: &Tensor<T>) {
    let y_shape = y.shape();
    let x_shape = x.shape();
...
pub fn rope<T: Copy + Clone + Default + Into<f32> + From<f32> + std::ops::Mul<Output = T> + std::ops::Sub<Output = T> + std::ops::Add<Output = T> + std::fmt::Debug>(y: &mut Tensor<T>, start_pos: usize, theta: f32) {
    let shape = y.shape();
    assert!(shape.len() == 3);
    let seq_len = shape[0];
    let n_heads = shape[1];
    ...
    for tok in 0..seq_len {
        ...
        for head in 0..n_heads {
            for i in 0..d / 2 {
                ...
                data[tok * n_heads * d + head * d + i + d / 2] = b * T::from(cos) + a * T::from(sin);
            }
        }
    }
}
...
pub fn causal_softmax<T: Copy + Clone + Default>(y: &mut Tensor<T>) {
    let shape = y.shape();
    let nbytes = convert_type::<T>().nbytes() as isize;
...
pub fn random_sample<T: Copy + Clone + Default>(
    ...
    let mut indices = Tensor::<u32>::new(vec![0], &vec![]);
