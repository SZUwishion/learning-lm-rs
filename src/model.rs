...
// 全局变量，用于控制是否已经写入过文件
static HAS_WRITTEN: Lazy<AtomicBool> = Lazy::new(|| AtomicBool::new(false));
    ...
    > Llama<T>
{
    ...
    pub fn forward(&self, input: &Tensor<u32>, cache: &mut KVCache<T>) -> Tensor<T> {
        ...
        for layer in 0..self.n_layers {
            ...
            // println!("att_scores: {:?}", att_scores.data());
            ...
            if layer == 0 && !HAS_WRITTEN.load(Ordering::Relaxed) {
                let mut hidden_file = File::create("hidden_states.txt").unwrap();
                let mut attn_file = File::create("attn_scores.txt").unwrap();
                writeln!(
                    hidden_file,
                    "Layer {} hidden_states: {:?}",
                    layer,
                ...
                writeln!(
                    attn_file,
                    "Layer {} attention scores shape: [{}, {}, {}, {}]",
                    layer, self.n_kv_h, n_groups, seq_len, total_seq_len
                )
                .unwrap();
                writeln!(attn_file, "Attention scores:").unwrap();

                // 重塑数据以匹配形状
                let att_data = att_scores.data();
                for h in 0..self.n_kv_h {
                    for g in 0..n_groups {
                        writeln!(attn_file, "Head {} Group {}:", h, g).unwrap();
                        for s in 0..seq_len {
                            let start_idx = (h * n_groups * seq_len * total_seq_len)
                                + (g * seq_len * total_seq_len)
                                + (s * total_seq_len);
                            let end_idx = start_idx + total_seq_len;
                            writeln!(
                                attn_file,
                                "  Sequence {}: {:?}",
                                s,
                                &att_data[start_idx..end_idx]
                            )
                            .unwrap();
                        }
                    }
                ...
                HAS_WRITTEN.store(true, Ordering::Relaxed);
            }
        }
        ...
        Tensor::<T>::new(logits.data().to_vec(), &vec![self.vocab])
    }
    ...
    pub fn chat(
        ...
        loop {
            ...
            let mut result = self.generate(&input_ids, max_len, top_p, top_k, temperature);
...
pub fn self_attention<T: Copy + Clone + Default + Into<f32> + std::fmt::Debug>(
    ...
    let mut att_output = Tensor::<T>::default(&vec![seq_len, n_kv_h, n_groups, dqkv]);
    ...
    for h in 0..n_kv_h {
        for g in 0..n_groups {
            ...
            let q_slice = Tensor::new(q_data, &vec![seq_len, dqkv]);
            ...
            let k_slice = Tensor::new(k_data, &vec![total_seq_len, dqkv]);
            ...
            // 2. 应用因果掩码和softmax
            OP::causal_softmax(&mut score_slice);
            ...
            let v_slice = Tensor::new(v_data, &vec![total_seq_len, dqkv]);

            let mut output_data = Tensor::<T>::default(&vec![seq_len, dqkv]);
            OP::matmul(&mut output_data, 0.0, &score_slice, &v_slice, 1.0);
            for s in 0..seq_len {
                ...
                unsafe {
                    output_slice
                        .data_mut()
                        .copy_from_slice(output_data.slice(s * dqkv, &vec![dqkv]).data());
                }
            }
        }
    }
